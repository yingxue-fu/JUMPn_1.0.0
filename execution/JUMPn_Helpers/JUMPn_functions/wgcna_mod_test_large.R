...
#input_exp_matrix='norm_exp_matrix.csv'
#col_offset=1

#min_cluster_size=30
#min_cluster_dist=0.2
#min_kME=0.7
...
execute_wgcna <- function(input_exp_matrix,network_type,outputDir,col_offset,min_cluster_dist,min_cluster_size,min_kME,scale_free_R_sq,non_scale_free_beta,metaFile){
  ...
  if (sum(-sign(sft$fitIndices[,3])*sft$fitIndices[,2]>scale_free_R_sq)==0) { # not scale free for beta < 20
    ...
    softPowerRow = which(-sign(sft$fitIndices[,3])*sft$fitIndices[,2]>scale_free_R_sq)[1]
  ...
  if (is.null(metaFile)==FALSE) {
    ...
    if (extension == 'xlsx'){                                                                            ### If the User File is .xlsx ...
      grp <- read_excel(metaFile)                                                               ### Call read_excel function on the user input file
    ...
    # meta data
    pData = as.data.frame(grp)
    print(nrow(pData))
    print(length(as.vector(unlist(grp[,1]))))
    rownames(pData)=as.vector(unlist(grp[,1]))
    # assay data
    assayData=t(mes3)
    ...
    if (sum(new_rownames==rownames(pData))==nrow(pData)) {
      ...
      for (i in 1:length(colUsed)) { # for each factor: 1 file 1 factor
          ...
          #print('Check3Passed')
          png(paste(outputDir,'/eigengene_vs_',colnames(pData)[colUsed[i]],'.png',sep=''),1200,1000)
          ...
          for (k in 1:nrow(egM)) { # for each factor: 1 cluster per page
            ...
            stripchart(egM[k,]~pData[,colUsed[i]], vertical = TRUE, method = "jitter", add = TRUE, pch = 50, col = 'blue')
          }
          dev.off()
        #}
      }
