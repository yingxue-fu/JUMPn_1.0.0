    ...
    if (length(filtered_pathways) >= 1){
      ...
      if (is.null(now_list)==FALSE){
        ...
        if (length(overlapped_gene_list) >= 1){ 
          ...
          total_df <- unique(cbind(total_df, 'BH FDR' = p.adjust(p_val,'BH'), '# Genes in Pathway'=pathway_length,'# Genes in Cluster/Module' = length(query_genes), '# Genes Overlapped'=unlist(overlapped_lengths)))[,1:7]
