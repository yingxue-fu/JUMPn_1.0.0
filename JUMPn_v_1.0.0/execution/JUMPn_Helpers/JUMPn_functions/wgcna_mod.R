...
#-----------------------------------------------------------------------------
execute_wgcna <- function(input_exp_matrix,network_type,outputDir,col_offset,min_cluster_dist,min_cluster_size,min_kME,scale_free_R_sq,non_scale_free_beta){
  ...
  if (sum(-sign(sft$fitIndices[,3])*sft$fitIndices[,2]>scale_free_R_sq)==0) { # not scale free for beta < 20
    ...
    softPower = which(sft$fitIndices[,2]>scale_free_R_sq)[1]
  }
  ...
  for (i in 1:length(tmp))
  {
    ...
    plot(MEn[,colcn],type='l',ylab='Relative level',xlab='',ylim=c(min(MEn[,colcn])-0.5,max(MEn[,colcn])+0.5),main=paste("Cluster",i,' (n=',length(tb[mergedColors==group,1]),')',sep=''))
